...
from lib.jdconfig.jdconfig import Config
...
def test_feature_unit(conf, key_array):
    ...
    if err1 < ERROR:
        ...
        logger.info('similarity is not expected, please check')
    # negative pairs
    ...
    if err1 < ERROR and err2 < ERROR:
        logger.info('[===============feature unit is ok ================]')
        return True
...
def detect_align_video(detector, key_imarray, align_shape=[112, 112]):
        ...
        for item in ldmks:
            for ldmk in item:
                ...
                if key not in res_dict:
                    ...
                    filename_temp = uuid.uuid4().hex
                    res_dict[filename_temp] = aligned_img
    return res_dict

if __name__ == '__main__':
    from service_conf import align_conf, feature_conf
    key_imarray = test_align_unit(align_conf)
    IS_OK = test_feature_unit(feature_conf, key_imarray)
    if IS_OK:
        logger.info('[=============face demo is ok============]')
    else:
        logger.info('[=======face demo is not ok plese check=======]')
